...
const LoginPage: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      if (isRegister) {
        setError(err.message || "Failed to create a new admin identity. Check password strength (min 6 characters) or if the email is already in use.");
      } else {
        setError("Invalid credentials. Please verify your email and password or use Onboarding Mode to register a new account.");
      }
    } finally {
      setLoading(false);
    }
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          >
            {loading ? (
              ...
              <>
                <span>{isRegister ? "Register Admin Node" : "Enter Terminal"}</span>
          ...
          <div className="text-center mt-6">
            ...
            >
              {isRegister ? "Already Registered? Sign In" : "Initialize New Account / Sign Up"}
            </button>
          </div>
        </form>
