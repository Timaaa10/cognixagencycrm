...
const Settings: React.FC<SettingsProps> = ({ onProfileUpdate, theme, onThemeChange, userProfile }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (user) {
        ...
        const localKey = localStorage.getItem('cognix_gemini_api_key') || '';
        ...
        if (a) {
          setAgency({
            ...
            geminiApiKey: a.geminiApiKey || localKey
          });
        } else {
          setAgency(prev => ({
            ...prev,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (agency.geminiApiKey) {
        localStorage.setItem('cognix_gemini_api_key', agency.geminiApiKey);
      } else {
        localStorage.removeItem('cognix_gemini_api_key');
      }
      if (isAdmin) {
      ...
      setModalConfig({
        ...
        message: 'Global parameters, AI key activations, and administrative identity updated successfully.'
      });
    } catch (err) {
      console.error(err);
      setModalConfig({
      ...
      <div className="grid gap-8">
        ...
        <section className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
          <div className="px-8 py-5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex justify-between items-center">
            <h3 className="font-black text-slate-900 dark:text-slate-300 text-[10px] uppercase tracking-[0.2em]">AI Engine Activation</h3>
            <span className={`text-[8px] font-black px-2.5 py-1 rounded uppercase tracking-widest ${agency.geminiApiKey || localStorage.getItem('cognix_gemini_api_key') ? 'text-emerald-500 bg-emerald-50 dark:bg-emerald-950/30 font-bold' : 'text-amber-500 bg-amber-50 dark:bg-amber-950/30 font-bold'}`}>
              {agency.geminiApiKey || localStorage.getItem('cognix_gemini_api_key') ? 'Active Node' : 'Inactive'}
            </span>
          </div>
          <div className="p-8 space-y-5">
            <p className="text-slate-500 dark:text-slate-400 text-xs leading-relaxed">
              When downloading the project or deploying to custom production environments, the pre-configured development API key is inactive. To enable real-time <strong>AI Lead Scoring</strong> and <strong>Strategic Analytics Advice</strong>, input your Gemini API key below. You can obtain a free API key from the <a href="https://aistudio.google.com/" target="_blank" rel="referrer noopener" className="text-blue-500 hover:underline font-bold">Google AI Studio Console</a>.
            </p>
            <div>
              <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1 mb-2">Gemini API Key</label>
              <input 
                type="password" 
                className="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-xl outline-none focus:ring-4 focus:ring-blue-500/5 focus:border-blue-500 dark:focus:border-blue-500/30 text-slate-900 dark:text-slate-100 font-mono text-sm"
                value={agency.geminiApiKey || ''}
                onChange={(e) => setAgency({ ...agency, geminiApiKey: e.target.value })}
                placeholder="AIzaSy..."
              />
              <span className="text-[9px] text-slate-400 dark:text-slate-600 mt-2 block ml-1 font-medium">
                {isAdmin ? 'Saved globally in the Firestore database & locally in secure browser storage.' : 'Saved locally in your secure browser storage.'}
              </span>
            </div>
          </div>
        </section>
