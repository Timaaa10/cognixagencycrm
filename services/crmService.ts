...
export const crmService = {
  ...
  getGeminiApiKey: async (): Promise<string> => {
    // 1. Try to fetch from user's custom saved agency settings
    try {
      const settings = await crmService.getAgencySettings();
      if (settings && settings.geminiApiKey) {
        return settings.geminiApiKey;
      }
    } catch (e) {
      console.warn("Could not retrieve custom Gemini API key from settings:", e);
    }

    // 2. Try to fetch from localStorage directly
  ...
  generateStrategicAdvice: async (leads: Lead[], settings: AgencySettings | null) => {
    try {
      const apiKey = await crmService.getGeminiApiKey();
      if (!apiKey) {
        return "Gemini API Key is not configured. Please add your key in Settings to activate real-time Strategic AI Advice.";
      }
  ...
  scoreLeadWithAI: async (lead: Lead) => {
    try {
      const apiKey = await crmService.getGeminiApiKey();
      if (!apiKey) {
        throw new Error("Gemini API Key is not configured. Please add your key in Settings to analyze and score this lead.");
      }
